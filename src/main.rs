...
fn fill_dependencies(
    ...
    )
    {
        ...
        let include_type = match filename.chars().nth(0).expect(
        ...
        match include_type {
            IncludeType::Relative => {
                ...
                let cpath = expect_fmt!(
                    std::path::Path::new(&fullpath).canonicalize(),
                    "Path {:?} does not exist",
                    fullpath
                );
...
fn main() {
    ...
    let top_include_path = expect_fmt!(
        PathBuf::from(&opt.top_include).canonicalize(),
        "Top include path `{}` doesn't exist or is not a directory.",
        opt.top_include
    );
